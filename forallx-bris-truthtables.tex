...
\chapter{Formal validity and English arguments}
...
\item If the murder weapon was the dagger then Mrs.~Peacock  is guilty.
\item If the murder weapon was the rope then Mrs.~Peacock  is guilty.
...
\item[\therefore] The contract is not void.
\end{earg}
We said its TFL symbolisation was invalid. This was given by the valuation 
\begin{tabular}{cc}
$D$&$V$\\\hline
F&T
\end{tabular} To see if the argument is \emph{conceptually} valid, one should then investigate whether the valuations providing counterexamples to the formal validity are conceivable possibilities. Is it conceptually possible that the contract is void but not signed under duress? Or perhaps in this case one is interested in something like legal validity; so we have to investigate whether the possibility of it being void but not signed under duress is compatible with the legal laws. 
...
\end{center}
which is invalid. 
The problem isn't the symbolisation we gave. It is the best we could do \emph{in TFL}. The problem is that TFL is limited in the ``form'' it can see. We will improve on this when we move to First Order Logic in \S\ref{s:FOLBuildingBlocks}.
This argument is formally valid in virtue of its FOL form rather than TFL form. 


\todo{note I deleted the section on paradoxes of material implication and double negation}
%
