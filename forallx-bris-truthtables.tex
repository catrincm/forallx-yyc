...
When we do these in practice, our sentences can have many subsentences, consider, e.g.~$$\enot (B\eand(\enot B\eiff \enot A)))$$ A tool for keeping our truth tables easier to do is not to put all the subsentences out as separate entire columns to calculate, but to simply list the values underneath the main connective of the sentence. This is simply to keep the truth table more concise. 
...
\begin{itemize}
\item[]If Jones signed the contract under duress, then the contract is voidable. But since it was not signed under duress, the contract is not voidable.\footnote{
...
\item \label{itm:validity-TTs} Check if the TFL argument is valid.\begin{itemize}\item Using truth tables to look for a valuation providing a counter example.
...
\item[\therefore] The contract is not voidable.
\end{earg}
...
And this allows us to read of a possibility where the premises are true but the conclusion false:  It might be that the contract is voidable without having been made under duress. E.g.~mental illness, misrepresentation,\ldots. 
...
\item[] $\metaX\eif\metaZ$
\item[] $\enot\metaX$
\item[\therefore]$\enot\metaZ$
\end{earg}
\end{minipage}
\end{tabular}\end{highlighted}

We can check these with truth tables.


Each fallacy has a very closely connected valid argument. From an invalid argument, e.g.~\begin{earg}
...
\end{earg}Sometimes we see people who are plausibly providing the fallacious argument. We should help them by directing them towards the premise that would actually help them in their argument.
...
\begin{itemize}\item usually because the relationship between premises and
conclusion is poor
\item sometimes because the premises are not an independent
reason to believe the conclusion\end{itemize}
