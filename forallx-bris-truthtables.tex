...
We want to know whether a sentence of the form $\enot \metaX$ is true. This depends on whether $\metaX$ is true or not.
...
\subsection{Conditional}\label{s:IndicativeSubjunctive}
...
\end{earg}
The bartender finds out that this beer drinker is sixteen, and can then conclude that sentence \ref{bartender} is false. This will give us the rule:
\begin{itemize}
\item If $\metaX$ is true and $\metaY$ is false, then $\metaX\eif \metaY$ is false.
\end{itemize}
If he instead finds out that she's nineteen, then there is no conflict with the conditional, and we will say \ref{bartender} is true. This is the rule:
\begin{itemize}
\item If $\metaX$ is true and $\metaY$ is true, then $\metaX\eif \metaY$ is true.
\end{itemize}
Someone who is drinking coke doesn't need to have their age checked. Whether they're sixteen or nineteen, sentence \ref{bartender} will be true. 
\begin{itemize}
\item If $\metaX$ is false and $\metaY$ true, then $\metaX\eif \metaY$ is true.
\item If $\metaX$ is false and $\metaY$ false, then $\metaX\eif \metaY$ is true.
\end{itemize}
...
You can see a summary of these rules for ease of reference in the `Quick Reference Appendix' \S\ref{app.CharacteristicTTs}
...
The truth value of a negation is uniquely determined by the truth value of the unnegated sentence. The truth value of a conjunction is uniquely determined by the truth value of both conjuncts. The truth value of a disjunction is uniquely determined by the truth value of both disjuncts, and so on. To determine the truth value of some TFL sentence, we only need to know the truth value of its components. 
...
For each connective we need to describe how the truth value of a sentence with that as its main connective depends on the consistent parts. 
...
\end{tabular}
\end{center}
This connective is often explicitly called the \emph{material conditional}. It is stipulated to act according to these rules. 
...
But is it any good, as a surrogate for the conditionals we use in everyday language? Consider two sentences:
	\begin{earg}
...
Exactly how well the material conditional does at representing \emph{indicative} conditionals, however, is a philosophically contentious issue, and not one we can further discuss in this course. 
...
In \S\ref{s:CharacteristicTruthTables} we described how the truth values of sentences \metaX and \metaY should combine to determine the truth of a sentence such as $\metaX\eor\metaY$. 

We will now describe how to extend this reasoning to determine the truths of more complex sentences such as $(I\eif H)\eand H$. 
...
Consider the sentence `$(\enot I\eif H)\eand H$'. 
...
\end{tabular}
\end{center}
Our job is to fill out the truths of $(\enot I\eif H)\eand H$. 
...
\item For `b': Look at the column for $I$, and see it's T, so by our rule (T-line), we fill out a=F.
\item For `c': Look at the column for $I$, and see it's F, so by our rule (F-line), we fill out a=T.
\item For `d': Look at the column for $I$, and see it's F, so by our rule (F-line), we fill out a=T.
\end{itemize}

The next step is to consider $\enot I\eand H$. For this we will use the truth rule we specified for $\eand$:
\begin{center}
\begin{tabular}{ccccc}
