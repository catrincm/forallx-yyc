...
\end{earg}
The former uses the \define{indicative conditional} and is plausibly true. After all, we know that Kennedy was assassinated. The latter uses the \define{subjunctive (or counterfactual) conditional} and is plausibly false. Unless there was a conspiracy plot, if Oswald hadn't have killed Kenney then Kennedy wouldn't have been assassinated. 
...
Whilst we use the English `if\ldots then\ldots' for both these sentences, they are very different kinds of conditionals. The former is \define{indicative} and the latter is \define{subjunctive}, also often called {counterfactual}.
