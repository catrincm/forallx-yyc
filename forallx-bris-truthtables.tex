...
When we do these in practice, our sentences can have many subsentences, consider, e.g.~$$\enot\enot(A\eor (B\eand(\enot B\eiff \enot A)))$$ A tool for keeping our truth tables easier to do is not to put all the subsentences out as separate entire columns to calculate, but to simply list the values underneath the main connective of the sentence. This is simply to keep the truth table more concise. 
...
\factoidbox{
	The TFL sentences $\metaX_1, \metaX_2, \ldots, \metaX_n$ \define{entail} the sentence $\meta{C}$ iff there are no valuations where all of $\metaX_1, \metaX_2, \ldots, \metaX_n$ true and $\meta{C}$ false.
}
...
\factoidbox{
		$\metaX_1, \metaX_2, \ldots, \metaX_n$ entail $\meta{C}$ iff $\metaX_1, \metaX_2, \ldots, \metaX_n \therefore \meta{C}$ is valid.
	}
...
%	$\metaX_1, \metaX_2, \ldots, \metaX_n$ entail $\meta{C}$ iff $\metaX_1, \metaX_2, \ldots, \metaX_n \therefore \meta{C}$ is valid.
%}
%Here's why. If $\metaX_1, \metaX_2, \ldots, \metaX_n$ entail $\meta{C}$, then there is no valuation which makes all of $\metaX_1, \metaX_2, \ldots, \metaX_n$ true whilst making $\meta{C}$ false. This means that it is \emph{logically impossible} for $\metaX_1, \metaX_2, \ldots, \metaX_n$ all to be true whilst $\meta{C}$ is false. But this is just what it takes for an argument, with premises $\metaX_1, \metaX_2, \ldots, \metaX_n$ and conclusion $\meta{C}$, to be valid!

In short, we have a way to test for the validity of English arguments. First, we symbolize them in TFL, as having premises $\metaX_1, \metaX_2, \ldots, \metaX_n$, and conclusion $\meta{C}$. Then we test for entailment using truth tables. 

\subsection{The double-turnstile}
We are going to use the notion of entailment rather a lot in this course. It will help us, then, to introduce a symbol that abbreviates it. Rather than saying that the TFL sentences $\metaX_1, \metaX_2, \ldots$ and $\metaX_n$ together entail $\meta{C}$, we will abbreviate this by:
...
Observe: $\metaX \entails \meta{C}$ iff there is no valuation of the atomic sentences that makes $\metaX$ true and $\meta{C}$ false. 

Observe: $\metaX \eif \meta{C}$ is a tautology iff there is no valuation of the atomic sentences that makes $\metaX \eif \meta{C}$ false. Since a conditional is true except when its antecedent is true and its consequent false, $\metaX \eif \meta{C}$ is a tautology iff there is no valuation that makes $\metaX$ true and $\meta{C}$ false. 

Combining these two observations, we see that $\metaX \eif \meta{C}$  is a tautology iff  $\metaX \entails \meta{C}$. But there is a really, really important difference between `$\entails$' and `$\eif$':
...
\item Suppose that $(\metaX\eand\metaY)\eif\meta{C}$ is neither a tautology nor a contradiction. What can you say about whether $\metaX, \metaY \therefore\meta{C}$ is valid?
%The sentence is false on some line of a complete truth table. On that line, \metaX and \metaY are true and \meta{C} is false. So the argument is invalid.
\item Suppose that $\metaX$, $\metaY$ and $\meta{C}$  are jointly inconsistent. What can you say about $(\metaX\eand\metaY\eand\meta{C})$?
\item Suppose that \metaX is a contradiction. What can you say about whether $\metaX, \metaY \entails \meta{C}$?
%Since \metaX is false on every line of a complete truth table, there is no line on which \metaX and \metaY are true and \meta{C} is false. So the argument is valid.
\item Suppose that \meta{C} is a tautology. What can you say about whether $\metaX, \metaY\entails \meta{C}$?
%Since \meta{C} is true on every line of a complete truth table, there is no line on which \metaX and \metaY are true and \meta{C} is false. So the argument is valid.
