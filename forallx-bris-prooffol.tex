...
\part{Natural deduction for FOL (Non-examinable)}
\label{ch.NDFOL}
...
We can formalise this argument as: $$Lab,\;Lbc,\;Ma,\;\enot Mc\;\therefore\;\exists x\exists y(Mx\eand (Lxy\eand\enot Mx)).$$ And we can show it is valid. In \S\ref{s:orE} we wrote this in a pseudo-formal style.
\begin{pf}
...
\hypo{3}{Ma}
\hypo{4}{\enot Mc}
	\have{lem}{Mb\eor\enot Mb}\LEM
	\open
	\hypo{b}{Mb}
	\have{a}{Lbc\eand\enot Mc}\andI{2,b}
	\have{ab}{Mb\eand (Lbc\eand\enot Mc)}\andI{a,b}
		\have{ab2}{\exists y(Mb\eand (Lby\eand\enot My))}\existsI{ab}
		\have{abc}{\exists x\exists y(Mx\eand (Lxy\eand\enot Mx))}\existsI{ab2}
	\close
	\open
	\hypo{b2}{\enot Mb}
		\have{a}{Lab\eand\enot Mb}\andI{b2,1}
	\have{ab2}{Ma\eand (Lab\eand\enot Mb)}\andI{3,a}
		\have{ab22}{\exists y(Ma\eand (Lay\eand\enot My))}\existsI{ab2}
		\have{abc2}{\exists x\exists y(Mx\eand (Lxy\eand\enot Mx))}\existsI{ab22}
	\close
			\have{con}{\exists x\exists y(Mx\eand Lxy\eand\enot Mx)}\orE{lem,b-abc,b2-abc2} 
\end{pf}
