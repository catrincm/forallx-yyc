...
\begin{quote}
	...
	And I could not have acted otherwise. 
...
Many arguments start with premises, and end with a conclusion, but not all of them. It might start with the conclusion:
	\begin{quote}\underline{We should not have a second Brexit referendum}.
...
Or it might have been presented with the conclusion in the middle:
	\begin{quote}
		...
		\underline{the voters deserve a further say on any final deal agreed with
...
\problempart
Now see if you can also identify the premises of the arguments in the previous part. 
%
For example:
\begin{earg}
	\item 
	\begin{earg}
		\prem It is sunny.
		\conc I should take my sunglasses.
	\end{earg}
\end{earg}
...
We said that arguments where the conclusion is a consequence of the premises are called valid, and those where the conclusion isn't a consequence of the premises are invalid.

	\factoidbox{
		An argument is \define{valid} if and only if the conclusion is a consequence of the premises. That is, there is no case where all the premises are true but the conclusion not true. 
	}

	\factoidbox{
		An argument is \define{invalid} if and only if it is not valid. That is, there is some case where all the premises are true and the conclusion false.
	}
