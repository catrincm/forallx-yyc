...
\hypo{a}{A}
\have{bc}{B\eand C}\by{Modus Ponens (which will be called $\eif$E)}{abc, a}
\have{b}{B}\andE{bc}
\end{pf}
...
%		\close
	...
	\end{pf}}
This is obviously a bit clunkier to write down than our previous rules, but the point is fairly simple. Suppose we have some disjunction, $\metaX \eor \metaY$. Suppose we have two conditionals,
%subproofs, 
showing us that $\metaZ$ follows from 
%the assumption that 
$\metaX$, and that $\metaZ$ follows from 
%the assumption that 
$\metaY$. Then we can infer $\metaZ$ itself. As usual, there can be as many lines as you like between $m$, $i$ and $j$.
...
\have{ndconc}{\enot D \eif ((P \eand D) \eor (P \eand \enot D))}{\ci{b-abo}}
	\have{con}{(P \eand D) \eor (P \eand \enot D)}\orE{lem,dconc,ndconc}
\end{pf}
...
Our Law of Excluded Middle, Disjunction Elimination and Conditional Introduction rules allow us to show this. 
%They write up 
We can demonstrate this in the following argument, which we present in a pseudo-formal style. (The gaps in the vertical line are just results of our text being too long to fit on a line. Please ignore them.)
\footnote{
...
\begin{pf}
	\have{lem}{\parbox{.8\textwidth}{{Bob is either happy or he's not happy}}}%\LEM
	\open
	\hypo{b}{\parbox{.8\textwidth}{{Suppose Bob is happy}}}
	\have{ab}{\parbox{.8\textwidth}{{Then happy Bob is looking at not-happy Charlie}}}
	\have{abc}{\parbox{.8\textwidth}{{So someone who is happy is looking at someone who is not happy}}}\vspace*{1em}
	\close
...
\have{babc}{\parbox{.8\textwidth}{{If Bob is not happy then someone who is happy is looking at someone who is not happy}}}\vspace*{1em}
	\have{con}{\parbox{.8\textwidth}{Therefore, someone who is happy is looking at someone who is not happy.}}%\orE{lem,b-abc,b2-abc2} 
\end{pf}
...
%However, we do need to state a rule for negation introduction. 
The rule 
for negation introduction
is very simple: if assuming something leads you to a contradiction, then the assumption must be wrong. This thought motivates the following rule:
\factoidbox{\begin{pf}
%\open
%	\hypo[m]{a}{\metaX}
%	\ellipsesline
...
\subsection{Negation Elimination} % New by SP
The elimination rule for negation is very similar to the introduction rule: if assuming that something is false leads you to a contradiction, then that assumption must be wrong --- and so that `something' must in fact be true.
\factoidbox{\begin{pf}
\have[m]{naxx}{\enot\metaX \eif \ered}
\have[\ ]{na}{\metaX}\ne{naxx}
\end{pf}}
This is sometimes called the method of \emph{proof by contradiction}, or \emph{indirect proof}. It allows us to prove something by assuming its negation and showing that it leads to contradiction. This technique is very common in mathematics.\footnote{
