...
\begin{center}
\begin{tikzpicture}
\node (A) [alice,evil,sword,minimum size=1cm] {Alice};
\node (B)[bob,monitor,evil,sword,minimum size=1cm,right=1cm of A] {Bob};
\node[charlie,monitor,female, minimum size=1cm,below=.8cm of A] {Cathy};
\node[businessman,minimum size=1cm,below=.8cm of B] {Denny};
\end{tikzpicture}

\begin{ekey}\item[\text{domain}] People in above picture (Alice, Bob, Cathy and Denny)
\item[Hx] \gap{x} has horns (Alice and Bob)
\item[Sx] \gap{x} carrying a sword (Alice, Bob and Denny)
\item[Cx]\gap{x} is looking at a computer (Bob and Cathy)
%\item[Gx] nothing
\end{ekey}
\end{center}
...
What about $\forall x Sx$? This means that $Sx$ is true under any choice of a person for $x$. Let's go through them. \begin{center}
\begin{tabular}{ccccc}
&$x\mapsto \text{Alice}$&$x\mapsto \text{Bob}$&$x\mapsto \text{Cathy}$&$x\mapsto \text{Denny}$\Bstrut\\\hline\Tstrut
$Sx$ is: &T&T&F&F
\end{tabular}
...
$Hx$ is: &T&T&F&F\\
$Cx$ is: &F&T&T&F\\
$Hx\eor Cx$ is: &T&T&T&F\\
$Sx\eif (Hx\eor Cx)$ is: &T&T&T&T
\end{tabular}
\end{center}So $Sx\eif (Hx\eor Cx)$  is true under every assignment of an object to the variable $x$. And so $\forall x(Sx\eif (Hx\eor Cx))$ is true. 



We have to tread more carefully once we start having multiple quantifiers. But let's just talk through some cases to start.

Consider a new interpretation: \begin{center}
\begin{tikzpicture}
\node (atom1) at (0,2) {1};
\node (atom2)[draw] at (2,2) {2};
\node (atom3) at (0,0) {3};
\node (atom4)  [draw] at (2,0) {4};
...
\begin{center}
 ...
 &$x\mapsto 1$& $x\mapsto 2$& $x\mapsto 3$& $x\mapsto 4$\Bstrut\\\hline\Tstrut
$Sx$ is: &F&T&F&T\\
$\exists y Rxy$ is:&T&T&T&T\\
$Sx\eif \exists y Rxy$ is: &T&T&T&T
\end{tabular}
...
One final example: $\forall x\forall y Rxy$. To check this we will need to consider all choices for $x$ and all choices for $y$ and check $Rxy$ is true on all of them. There are 16 such choices. But we won't have to go through them all: it'll be false. Consider $x\mapsto 2$ and $y\mapsto 4$. $Rxy$ is false under this variable assignment. Thus $\forall yRxy$ is false on the variable assignment $x\mapsto 2$. And so $\forall x\forall y Rxy$ is false in the interpretation. 
 
 Let's now give a formal definition of the idea we've been using here. Quantified formulas like $\forall y Rxy$ still need to be given truth conditions \emph{relative to a variable assignment}, because we'll need to specify an assignment of an object for the free variable $x$. So we define when $\forall \metav\metaX$ is true under a variable assignment $\alpha$, which might be, e.g.~$x\mapsto 1$. To give a general definition, though we might also consider whether $\forall y Rxy$ is true under a variable assignment $x\mapsto 1, y\mapsto 2$. This is slightly odd: we're considering $\forall y Rxy$ but have been told who $y$ refers to already. However when we evaluate it we simply ignore whatever our given variable assignment tells us about $y$: we consider variable assignments that modify the assignment by changing what is assigned to $y$. So, e.g.~$\exists x Sx$ is true under variable assignment $x\mapsto 1$: there's a modification of this variable assignment which gives us $x\mapsto 2$ under which $Sx$ is true.  Sentences which have all variables bound, don't have their truth value dependent on any variable assignment anymore, they're simply true or false in the interpretation.
 
 Now for our formal definition:
\factoidbox{
$\forall \metav\metaX$ is true under a variable assignment $\alpha$\\
\textbf{iff} $\metaX$ is true under \emph{every} variable assignment that is the result of modifying/extending $\alpha$ with a choice of an object in our domain for $x$. \\

$\exists \metav\metaX$ is true under a variable assignment $\alpha$\\
\textbf{iff} $\metaX$ is true under \emph{some} variable assignment that is the result of modifying/extending $\alpha$ with a choice of an object in our domain for $x$. 
}
...
means that there is no interpretation in which all of $\metaX_1, \metaX_2, \ldots, \metaX_n$ are true and in which \meta{C} is false. Derivatively,
	$$\entails\metaX$$
...
\item $\metaX_1, \metaX_2, \ldots \metaX_n \therefore \meta{C}$ is \define{valid in FOL} iff there is no interpretation in which all of the premises are true and the conclusion is false; i.e., $\metaX_1,\metaX_2,\ldots \metaX_n \entails\meta{C}$. It is \define{invalid in FOL} otherwise.
...
\item The FOL sentences $\metaX_1,\metaX_2,\ldots, \metaX_n$ are \define{jointly consistent} iff there is some interpretation in which all of the sentences are true. They are \define{jointly inconsistent} iff there is no such interpretation.
...
and many more besides.
	...
	\factoidbox{
	To show that $\metaX_1, \metaX_2, \ldots, \metaX_n \therefore \meta{C}$ is invalid, it suffices to find an interpretation where all of $\metaX_1, \metaX_2, \ldots, \metaX_n$ are true and where $\meta{C}$ is false.
	
	That same interpretation will show that $\metaX_1, \metaX_2, \ldots, \metaX_n$ do not entail $\meta{C}$.
	
	It will also show that $\metaX_1, \metaX_2, \ldots, \metaX_n, \enot \meta{C}$ are jointly consistent.}
