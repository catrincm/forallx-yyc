...
We can now give the recursive definition of what it is to be a sentence of FOL: 
	...
	\begin{enumerate}
		...
		\item\label{item:quantifiers} 
		 If $\meta{A}(\ldots \meta{c}\ldots)$ is a sentence, where $\meta{c}$ is a name, then so is: \begin{itemize}
		 \item $\forall \meta{x}\meta{A}(\ldots \meta{x}\ldots)$ (so long as $\meta{x}$ does not appear already in $\meta{A}$.)
		 \item $\exists \meta{x}\meta{A}(\ldots \meta{x}\ldots)$ (so long as $\meta{x}$ does not appear already in $\meta{A}$.)
		 \end{itemize}
%		If \begin{itemize}
%		\item \meta{A} is a sentence, 
%		\item \meta{x} is a variable that does not appear in $\meta{A}$, 
