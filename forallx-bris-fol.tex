...
Since sentence \ref{quan1} is about coins that are both in my pocket \emph{and} that are quarters, it might be tempting to symbolize it using a conjunction. However, the sentence `$\forall x(Px \eand Qx)$' would symbolize the sentence `every coin is both a quarter and in my pocket'. This obviously means something very different than sentence \ref{quan1}. And so we see:
	\factoidbox{
		A sentence can be symbolized as $\forall x (\meta{F}x \eif \meta{G}x)$ if it can be paraphrased in English as `every F is G', `there is some F that is G', `some F is G', or `there is at least on F that is a G'.
	}
...
But which expressions are sentences?
...
\factoidbox{ If $\meta{R}$ is an $n$-place predicate and $\meta{c}_1, \meta{c}_2, \ldots, \meta{c_n}$ are names, then $\meta{R c}_1 \meta{c}_2 \ldots \meta{c}_n$ is an atomic sentence.}

We can now give the recursive definition of what it is to be a sentence of FOL: 
	\factoidbox{\label{FOLsentences}
	\begin{enumerate}
		...
		\item\label{item:quantifiers} 
		 If $\metaX(\ldots \meta{c}\ldots)$ is a sentence, where $\meta{c}$ is a name, then so is: \begin{itemize}
		 \item $\forall \meta{x}\metaX(\ldots \meta{x}\ldots)$ (so long as $\meta{x}$ does not appear already in $\metaX$.)
		 \item $\exists \meta{x}\metaX(\ldots \meta{x}\ldots)$ (so long as $\meta{x}$ does not appear already in $\metaX$.)
		 \end{itemize}
%		If \begin{itemize}
%		\item \metaX is a sentence, 
%		\item \meta{x} is a variable that does not appear in $\metaX$, 
%		\item $\meta{A'}$ is the expression that is the result of replacing any occurrences of a name, $\meta{a}$, by $\meta{x}$.
%		\end{itemize} then $\exists \meta{x}\meta{A'}$ and $\forall \meta{x} \meta{A'}$ are sentences. 
	...
	description={A string of symbols in FPL that can be built up according to the recursive rules given on p.~\pageref{FOLsentences}}
	}
...
Why in \ref{item:quantifiers} did we have the restriction that $\meta{x}$ is not already in $\metaX$? This is to ensure that variables only serve one master at any one time (see \S\ref{s:MultipleGenerality}).
