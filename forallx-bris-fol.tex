...
We can now give the recursive definition of what it is to be a formula of FOL. 
	...
	\newglossaryentry{formula of FOL}
	{
	name=sentence of FOL,
	description={A string of symbols in FPL that can be built up according to the recursive rules given on p.~\pageref{FOLformula}}
	}
...
\begin{earg}
	\item[] $\exists x({M}{x} \eand \forall y({P}{y} \eif {S}{y,x}))$
	\item[] $\forall y({P}{y} \eif \exists x({M}{x} \eand {S}{y,x}))$
\end{earg}
