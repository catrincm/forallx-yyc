...
In \S\ref{s:SymbolisingSimpleFOL} we discussed how to symbolise some sentences. We now want to take this further and be able to symbolise more complex sentences.

We again follow the same kind of strategy that we used for TFL in \S\ref{s:SymbolisingComplexTFL}:
...
So we use:
\begin{itemize}
\item For every \metaPredicate, \metav, \metaX(\metav). 
\end{itemize}
...
\hline
For every \metaPredicate, \metaX holds of it& $\forall x(Fx \eif Gx)$\\
...
Consider 	
\begin{earg}
	\item[\ex{loves everyone}] Catrin loves everyone.
\end{earg}

This is paraphrased as `For every person, $x$, Catrin loves $x$'.

I have introduced

Consider 	
\begin{earg}
	\item[\ex{and}] Joe and Fred are happy.
\end{earg}


Consider 
\begin{earg}
	\item[\ex{and}] Joe is rich and powerful. 
\end{earg}
