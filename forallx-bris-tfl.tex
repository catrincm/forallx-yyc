...
\begin{earg}
\item[\ex{mathamb1}] $9 + (3 \times 4) \hspace*{5mm}(=48)$
\item[\ex{mathamb2}] $(9+3) \times 4 \hspace*{5mm}(=21)$
\end{earg}
In TFL,
\begin{earg}
...
\end{enumerate}
By our first rule, we know that $A$ is a sentence. This second rule allows us to conclude that $\enot A$ is also a sentence. We could then apply it again and conclude that $\enot\enot A$ is also a sentence.
...
\end{enumerate}
By rule 1, $B_1$ and $D$ are both sentences. So this third rule allows us to conclude that $(B_1\eand D)$ is a sentence. We might then apply rule 2 to conclude that $\enot(B_1\eand D)$ is also a sentence. 
\begin{center}
\begin{forest}
...
One more example: consider $\enot (P \eand \enot (\enot Q \eor R))$ we can check this is a sentence by drawing the following formation tree:
\label{S:formationtree}
\begin{center}
\begin{forest}
	[$\mainconnective{\enot}\,  (P \eand \enot (\enot Q \eor R))$
		[$(P \,\mainconnective{\eand}\,  \enot (\enot Q \eor R))$
			[$P$]
			[$\mainconnective{\enot}\,   (\enot Q\eor R)$
...
\end{center}
each of the steps here tracks one of the rules of what it is to be a sentence. So we can conclude that this is a sentence of TFL. This also helps us see how to read it.
The will be important when we consider the circumstances under which a particular sentence would be true or false. The sentence $(A\eand ())$ is true if and only if the sentence $\enot \enot D$ is false, and so on through the structure of the sentence, until we arrive at the atomic components. We will return to this point in Part~\ref{ch.TruthTables}.
...
\factoidbox{The \define{main connective} of sentence is the last connective that was introduced in the construction of the sentence.} In the case of $((\enot E \eor F) \eif \enot\enot G)$, the main connective is $\eif$. Here we can see that the whole sentence can be described in the form $(\metaX\eif\metaY)$ with both $\metaX$ and $\metaY$ being complete sentences. That's enough to see that $\eif$ is the main operator. 
...
The recursive structure of sentences in TFL also allows us to give a formal definition of the \emph{scope} of an instance of connective. The scope of a connective instance is the subsentence(s) for which it is the main connective. One can use formation trees to work this out. 
...
\begin{center}
	\begin{forest}
[$(P \,\mainconnective{\eand}\,  (\enot (R \eand B) \eiff Q))$
	[$P$]
	[$(\enot (R \eand B) \,\mainconnective{\eiff}\,  Q))$
		[$\mainconnective{\enot}\,   (R \eand B)$
			[$(R\,\mainconnective{\eand}\,   B)$
				[$R$]
				[$B$]
			]
		]
...
The scope of the $\enot$ is $\enot (R \eand B)$. It is the subsentence in which $\enot$ is the main logical operator. We have worked that out by drawing the formation tree and finding the lowest sentence in which $\enot$ appears. 
...
More generally:
	\factoidbox{The \define{scope} of an instance of a connectiveis the subsentence for which it is the main connective.}
